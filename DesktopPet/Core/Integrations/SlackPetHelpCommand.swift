...
enum SlackPetHelpCommand {
    ...
    • **截屏档位（Slack）**：`!pet screen off` / `main` / `secondary` 可在**本机总开关非「关」**时远程切换「关 / 主屏 / 副屏」。总开关为「关」时**不能**用 Slack 远程打开，但可发 `!pet screen pick main` / `pick secondary` 或「截屏目标主屏 / 副屏」**仅记录**下次按哪块物理屏截（供远程点屏等）。
