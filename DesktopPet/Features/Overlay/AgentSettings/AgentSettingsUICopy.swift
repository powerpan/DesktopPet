...
enum AgentSettingsUICopy {
    ...
    /// 系统设置里巡逻「间隔 / 边距 / 贴近前台」三项的脚注。
    static func settingsPanelPatrolTuneFooter(testing: Bool) -> String {
        if testing {
            return "**间隔**：定时器周期，改后若巡逻已开启会立即重启计时器。**靠边距**：落点相对 `visibleFrame`（不含菜单栏与 Dock）的内缩。**贴近前台窗**：刻度 0…100 = 每次 tick **掷骰**，以 `k/100` 概率在「前台区域」矩形（与调试遮罩红框同源）**内**对宠物原点均匀随机；否则 **100% 落在红外**（不与红区相交）。**0**=永不进红；**100**=永远在红内随机。取不到红区几何时退回整屏均匀随机。"
        }
        return "**巡逻间隔**就是猫猫隔多久挪一次窝。**靠边距**大一点，就更不容易贴边、贴 Dock。**贴近前台窗**：**0～100** 是掷骰概率——比如 **5** 大约 **5%** 这一下会进「红色那块前台区域」里随便落个点，**95%** 这一下保证在红色外面逛；**0** 永远不进红；**100** 永远在红里面随机落。旁边 `n%` 就是概率。"
    }
    ...
    static func triggersIntroLinesTesting() -> [String] {
        [
            ...
            "轻点气泡仅关闭气泡；**长按约 1 秒**会关闭气泡、以该旁白为上下文新建一个手动会话频道，并打开对话面板续聊。",
    ...
    static func triggersIntroLinesUser() -> [String] {
        [
            ...
            "点一下气泡会先关掉它；想带着这句旁白开新聊天继续聊的话，在气泡上**按住约一秒**就好。",
