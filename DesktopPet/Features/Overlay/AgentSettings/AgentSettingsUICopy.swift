...
enum AgentSettingsUICopy {
    ...
    static func settingsPanelPatrolTuneFooter(testing: Bool) -> String {
        if testing {
            return "**间隔**：定时器周期，改后若巡逻已开启会立即重启计时器。**靠边距**：落点相对 `visibleFrame`（不含菜单栏与 Dock）的内缩，越大越远离屏幕边缘与 Dock。**贴近前台窗**：每次 tick 在均匀随机原点基础上，以该概率与「前台应用大窗上沿（带抖动）」做一次线性混合；0% 则完全不在落点上参考前台窗。"
        }
        return "**巡逻间隔**就是猫猫隔多久挪一次窝。**靠边距**大一点，就更不容易贴边、贴 Dock；小一点活动范围更宽但要小心别挡菜单。**贴近前台窗**的百分比越高，越常往你正在用的窗口上头凑一凑；拉到 0 就完全不跟前台窗掺和，只在空桌面里随机逛。"
    }
