...
enum AgentSettingsUICopy {
    ...
    static func privacyScreenSnapPickerInline(testing: Bool) -> String {
        if testing {
            return "「关」：不跑截屏类自动化、菜单栏截屏旁白；远程点屏仍可在有屏幕录制权限时按 Slack 记录的显示器偏好截屏。「截取主/副屏」：按所选物理显示器截一张图（副屏需外接且系统可见）。"
        }
        return "选「关」就不跑截屏自动化和菜单栏截屏旁白；远程点屏在已授权时仍可按你在 Slack 里选的屏来截。「主屏 / 副屏」会真的截那一块显示器（副屏要接着且系统认得）。"
    }

    static func privacyAdvancedSwitchesFooter(testing: Bool) -> String {
        if testing {
            return "键盘总闸：关闭后，所有「键盘模式」类触发器都不会匹配子串。截屏档位为「关」时不跑截屏自动化与菜单栏截屏旁白；为「主/副屏」时仍须系统「屏幕录制」权限及已启用的截屏规则。Slack 可在总开关为关时发「截屏目标主屏」等仅记录显示器偏好。"
        }
        return "键盘总开关关着，所有键盘类触发都不匹配。截屏选「关」就不跑截屏自动化；选主/副屏要系统「屏幕录制」权限和对应规则。关着截屏时仍可在 Slack 里发「截屏目标主屏」这类话，只记下偏好、不真的开自动化。"
    }
    ...
    static func privacyScreenSnapAlertMessage(testing: Bool) -> String {
        if testing {
            return "开启「截取主屏」或「截取副屏」后，满足条件的「截屏」触发器会通过 ScreenCaptureKit 截取所选显示器画面，经缩放与 JPEG 压缩后，作为多模态请求的一部分发往你在「连接」里为**当前服务商**配置的 Base URL 与模型。画面可能包含屏幕上任何可见内容；请在会议或投屏场景改回「关」或关闭对应规则。默认不落盘原图。若模型不支持图像，应用会尝试自动改为纯文字重试一次。"
        }
        return "主/副屏打开后，截屏触发会拍所选显示器的一帧，压缩后发给当前模型，屏上能看见的都可能被看到；开会或投屏时请改回「关」或关掉规则。默认不把原图存磁盘；模型不认图时会自动试一次纯文字。"
    }
    ...
    static func connectionSlackFooter(testing: Bool) -> String {
        if testing {
            return "在 Slack 频道发送 `!pet new 标题` 可新建本地会话并绑定。首次连接某频道会跳过历史回放，仅同步之后的新消息。出站会同步当前绑定频道内你发送的 user/assistant 消息。"
        }
        return "在频道里发 `!pet new 标题` 可以新建本地会话并绑上。第一次连某个频道不会翻旧账，只同步之后的新消息；出站会把你发出去的对话同步回 Slack。"
    }
    ...
    static func integrationsRemoteClickBody(testing: Bool) -> String {
        ...
        return "在监控频道发 **`!pet click`** 或 **`!pet 点屏`**，也可以用「远程点屏」这类中文开头（词后面请加空格或标点，别和别的汉字粘在一起，免得误触）。七七会截一帧主屏或副屏，带上坐标标尺图发回线程（要屏幕录制和 Bot 能上传文件；失败时也会请你用文字报坐标）。你在**同一线程**回 **`50,50`** 或 **`x=0.5 y=0.5`** 这种格式即可。点完可以说 **继续** 再玩一轮，或 **结束** 收工。真正点屏幕需要 **辅助功能** 权限。大概五分钟没人理会自动超时。还可以在 Slack 里用指令记下「下次截哪块屏」，细节见测试模式下的长说明。"
    }

    static func integrationsRemoteClickSelfTest(testing: Bool) -> String {
        if testing {
            return "端到端自测建议：① 发 `!pet click` 见线程内坐标图；② 回 `50,50` 点击后应提示是否继续；③ 回 **继续** 应再发一张坐标图；④ 回 **结束** 应退出会话；⑤ 回 `x=120` 类越界应报错且不点击；⑥ 关闭辅助功能后应仅回帖提示授权。"
        }
        return ""
    }
    ...
    static func triggerEditorScreenSnapFooterLines(testing: Bool) -> [String] {
        if testing {
            return [
                ...
                "自动触发需打开「隐私」中的截屏档位（截取主屏或截取副屏），并授予屏幕录制。所选显示器经 ScreenCaptureKit 抓取后按「长边上界」缩放再 JPEG 编码（最大 2048px），仅在内存中上传；长边越大越利于认字，但请求体与耗时通常也会增加。",
        ...
        return [
            ...
            "自动截屏要在「隐私」里打开主/副屏档位，并给好屏幕录制权限。图只在内存里上传，不默认存盘。",
            "模型不认图时会自动试一次纯文字。",
        ]
    }
