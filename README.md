...
| 触发器 | `AgentTriggerEngine`：定时、随机空闲、键盘子串匹配（仅内存环形缓冲，不落盘原文）、前台应用名子串（`FrontmostAppWatcher`）；每条规则 `cooldown`；截屏类型为占位，当前**不会**请求截屏权限或上传图像。成功时写入旁白历史并由 `ExtensionOverlayController.showTriggerBubble` 展示气泡。 |
