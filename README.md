...
- **鼠标穿透**：菜单栏 **设置…** 与窗口右上角按钮共用同一开关（`SettingsViewModel.isClickThroughEnabled`）。开启时精灵区不参与命中（`PetSpriteView` 上 `allowsHitTesting(false)`），`PetRootContainerView` 在居中包络外 `hitTest` 返回 `nil`；包络/窗口边长由 `PetConfig.exteriorHitSide` 按「视觉边长 + 少量 slack」计算（避免再叠一大圈仍挡下层）。`PetContainerView` 底层铺 `Color.clear.allowsHitTesting(false)`，减少 ZStack 空白区误命中。
