...
以下为**发版或回归自测**时建议逐项核对；与 `README.md`、源码不一致处以仓库为准。

### 9.1 桌宠核心（已实现，回归时勾选）

- [x] 宠物窗口透明、无边框、顶层显示正常（`PetWindow` / `PetWindowController`）
- [x] 辅助功能权限引导流程可用（`AccessibilityOnboardingView`、延迟重检）
- [x] 键盘输入触发状态反馈（桌镜 + `PetStateMachine`）
- [x] 鼠标互动与巡逻基本稳定（阈值与边界以 `MouseTracker`、`ScreenGeometry` 为准）
- [x] 穿透与拖动切换正确（`PetRootContainerView`、设置联动）
- [x] Cmd+K 全局快捷键可用（`GlobalInputMonitor`）
- [x] 巡逻在可见桌面内夹紧，并支持贴近前台窗口等策略（见 `PetWindowController`）

### 9.2 后续 / 非阻断项（见 `docs/TODO.md`）
...
```text
DesktopPet/
├── App/                      # 入口、委托、协调器、路由与部分服务装配
├── Core/
│   ├── Window/               # 宠窗、叠加窗、扩展面板控制器等
│   ├── Input/                # 全局/本地键盘、鼠标采样、远程点击等
│   ├── Permissions/          # 辅助功能
│   ├── PetState/             # 状态机与巡逻调度
│   ├── Models/               # PetConfig、DeskMirror、交互事件等
│   ├── Animation/            # 状态到展示的占位/驱动
│   ├── Care/                 # 饲养数值与持久化
│   ├── Agent/                # 智能体客户端、会话、触发器、钥匙串等
│   ├── Capture/              # 截屏（SCK）与远程点击渲染
│   ├── Watch/                # 盯屏检测与持久化
│   └── Integrations/         # Slack 等集成
├── Features/
│   ├── PetView/              # 桌镜与宠窗内 UI
│   ├── Settings/             # 系统 Settings 场景中的桌宠设置
│   ├── Onboarding/           # 权限说明
│   └── Overlay/              # 饲养、对话、智能体工作台等叠加 UI
├── Resources/DeskMirror/     # 桌镜 PNG（Folder Reference）
└── Utils/                    # 日志、屏幕几何、Markdown 气泡等
```

仓库根目录另有 `self_test/`（脚本类自检）、`docs/`（本 PRD 与其它专题文档）。**Xcode 单元测试目标**若未加入工程，则不在上表中强行列出。

## 11. 里程碑

早期 M1～M5（窗口、权限、状态机、互动、设置）及饲养/智能体/截屏等扩展已在主分支持续交付；**未完成的产品增量**统一跟踪在 [`docs/TODO.md`](TODO.md) 与 [`docs/TODO_AGENT_AND_CARE.md`](TODO_AGENT_AND_CARE.md) 的待勾选条目中。
